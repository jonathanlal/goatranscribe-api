...
def main(input: dict) -> str:
    ...
    try:
        #wtf took 10mins to do this for 30second audio file on the plane (12kb/s)
        logging.info('stating openAI conntection')
        if is_large_file:
    ...
    upload_file_to_azure(transcript_file_name, transcript, user_id, metadata=metadata) #upload text transcript to azure
    ...
    if(stripe_balance != new_balance):
        update_audio_status(user_id, entry_key, "balance_mismatch")
        return json.dumps({entry_key: "balance_mismatch"})
